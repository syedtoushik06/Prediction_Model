...
required_packages <- c(
  ...
  "scales", "grid", "DT", "shiny", "hexbin", "kableExtra"
)
...
DT::datatable(
  ...
  options = list(
    ...
    buttons = list(
      ...
      list(extend = 'csv', className = 'btn-sm', filename = 'IMDB_movies'),
      list(extend = 'excel', className = 'btn-sm', filename = 'IMDB_movies'),
      list(extend = 'pdf', className = 'btn-sm', filename = 'IMDB_movies'),
...
```{r stats-calc, include=FALSE, cache=TRUE}
# Pre-calculate statistics
rating_mean <- round(mean(movies$Rating), 2)
rating_median <- round(median(movies$Rating), 2)
rating_sd <- round(sd(movies$Rating), 2)
rating_min <- min(movies$Rating)
rating_max <- max(movies$Rating)

votes_mean <- comma(round(mean(movies$Votes)))
votes_median <- comma(round(median(movies$Votes)))
votes_sd <- comma(round(sd(movies$Votes)))
votes_min <- comma(min(movies$Votes))
votes_max <- comma(max(movies$Votes))
```
...
<li><b>Mean:</b> <span style="color:#1F618D;">`r rating_mean`</span></li>
<li><b>Median:</b> <span style="color:#117A65;">`r rating_median`</span></li>
<li><b>Std Dev:</b> <span style="color:#BA4A00;">`r rating_sd`</span></li>
<li><b>Range:</b> <span style="color:#884EA0;">`r rating_min` - `r rating_max`</span></li>
</ul>
</div>
...
<ul style="font-size:16px;line-height:1.35;text-align:left; display:inline-block;">
<li><b>Mean:</b> <span style="color:#BA4A00;">`r votes_mean`</span></li>
<li><b>Median:</b> <span style="color:#117A65;">`r votes_median`</span></li>
<li><b>Std Dev:</b> <span style="color:#5D6D7E;">`r votes_sd`</span></li>
<li><b>Range:</b> <span style="color:#884EA0;">`r votes_min` - `r votes_max`</span></li>
</ul>
</div>
</div>
...
```{r correlation, include=FALSE, cache=TRUE}
correlation <- round(cor(movies$Rating, movies$Votes), 3)
cor_strength <- ifelse(abs(correlation) > 0.7, "Strong positive", 
                       ifelse(abs(correlation) > 0.4, "Moderate positive", "Weak"))
```

<div style="background-color:#E9F7EF; padding:18px 25px; border-radius:14px; text-align:center;">
<span style="font-size:19px;">
<b>Correlation coefficient (r):</b> <span style="color:#C0392B;font-size:25px;"><b>`r correlation`</b></span>
</span>
<br>
<b>Interpretation:</b>
<span style="color:#154360;">
`r cor_strength` relationship
</span>
</div>
...
```{r combined-visual, fig.width=16, fig.height=6, cache=TRUE}
...
```{r diagnostics, fig.width=13, fig.height=10, cache=TRUE}
...
plot1 <- ggplot(test_data, aes(x = Votes, y = Predicted)) +
  ...
  labs(title = "Actual vs Predicted Votes", x = "Actual Votes", y = "Predicted Votes",
       subtitle = "Perfect prediction line shown in red") +
  theme_minimal(base_size = 14) +
  theme(
...
```{r example-pred, cache=TRUE}
example_ratings <- seq(7.5, 9.5, by = 0.5)
pred_votes <- intercept + slope * example_ratings

prediction_table <- data.frame(
  "IMDB_Rating" = example_ratings,
  "Predicted_Votes" = comma(round(pred_votes, 0))
)
...
<div style="background-color:#FEF5E7; padding:20px 32px; border-radius:15px; font-size:19px;">
